...
- **モデル選択指標：PR-AUC**（全foldの平均で比較）。不調日が少数派であるため、クラス不均衡に対して正直な評価を返すPR-AUCを採用
- ROC-AUCはログ出力として残す
- 正例が train/test どちらかに0件の fold はスキップし、有効 fold のみで平均を算出
- TSCVはモデル選択のためだけに使い、選択後は全データで再学習して最終予測を出す

> **設計根拠**: 単一分割（例: 直近14日テスト）では正例が1-3件程度になりやすく、AUCの信頼区間が極めて広い。複数foldの平均を取ることで評価の安定性が大幅に向上する。Expanding Window方式はデータ量が少ない段階でも各foldの学習データを最大化できる。Rolling Window方式は分布シフトに強いが、少データ期では学習量不足になるため将来の検討課題とする。
