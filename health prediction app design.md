...
**睡眠セグメント派生特徴量（入力負担ゼロ・Section 7.1 参照）：**

- `nap_total_min`：仮眠の合計時間（分）。0なら仮眠なし。「短い主睡眠＋長い昼寝→不調」パターン検出
- `sleep_fragmentation`：睡眠セグメント数。分割睡眠・不眠傾向の指標
- `total_sleep_hours`：主睡眠＋仮眠の合計時間。1日の総睡眠量

> **既存特徴量との関係**: `sleep_hours_filled` は最長ブロック（主睡眠）ベースで維持。`total_sleep_hours` は仮眠を含む合計。両方保持することで「主睡眠の質」と「総睡眠量」の両面を捉える。
...
- **最長睡眠ブロックの起床日**をその日の睡眠として扱う
- 例：2/5 23:40 〜 2/6 06:30 → 2/6 の睡眠として扱う

### date_key の定義

- `date_key` は**最長睡眠ブロックの起床時刻を端末ローカルTZに変換した `YYYY-MM-DD`**
- Firestore に保存する timestamp は**UTC**
- 併せて `tz_at_wake`（例: `Asia/Tokyo`）を内部保存する（ユーザー非表示）
- 固定時間ウィンドウ（18:00〜12:00等）には依存しないため、夜勤者・昼夜逆転にも対応
...
「主睡眠」と「仮眠（昼寝）」を入力時に区別せず、**全睡眠セグメントをフラットに記録し、集約はML/表示の段階で行う**。

- 「主睡眠」「仮眠」は**生データの区分ではなく、派生概念**として扱う
- 記録層では区別なく全セグメントを保存し、集約層で最長ブロック（≒主睡眠）とそれ以外（≒仮眠）を自動分類する
- 固定時間ウィンドウ（18:00〜12:00）への依存を排除する

### 3層構造

```
記録層（生データ）     → 全睡眠セグメントをリストで保存
集約層（派生データ）   → totalSleepMin, longestBlock, segmentCount 等を算出
ML特徴量層            → 既存5特徴量（互換維持）+ 新規3特徴量
```

### 睡眠セグメントの集約ロジック

1. 全セグメントを取得
2. ギャップ30分未満のセグメントを結合 → 「ブロック」を形成（トイレ等の短時間覚醒をフィルタ）
3. **最長ブロック = 主睡眠**
4. それ以外のブロック = 仮眠（supplementary sleep）
...
```json
{
  ...
  "sleepSegments": [
    { "start": "2026-03-23T23:30:00", "end": "2026-03-24T07:00:00", "minutes": 450, "source": "auto" },
    { "start": "2026-03-24T13:00:00", "end": "2026-03-24T13:30:00", "minutes": 30, "source": "manual" }
  ],
  "sleepSummary": {
    "totalSleepMin": 480,
    "longestBlockMin": 450,
    "longestBlockStart": "23:30",
    "longestBlockEnd": "07:00",
    "segmentCount": 2,
    "napTotalMin": 30
  }
}
...
**後方互換性:** `sleep` フィールドは維持し、`longestBlock` の値で同期する。既存データは `sleepSegments` が無い状態で `sleep` フィールドにより動作し続ける。

### 対応パターン一覧

| パターン | セグメント | 最長ブロック | 仮眠合計 | total | 結果 |
|---------|----------|------------|---------|-------|------|
| 通常（23:00→7:00） | [480] | 480 | 0 | 480 | 既存と同じ |
| 通常+昼寝30分 | [480, 30] | 480 | 30 | 510 | 主睡眠と仮眠が自然に分離 |
| 分割睡眠（4h+2h） | [240, 120] | 240 | 120 | 360 | 合計6h（統合による9h問題を解消） |
| 夜勤（8:00→15:00） | [420] | 420 | 0 | 420 | ウィンドウ不要で正常取得 |
| 多相睡眠（3h×3） | [180,180,180] | 180 | 360 | 540 | fragmentation=3が特徴に |
| 徹夜+仮眠30分 | [30] | 30 | 0 | 30 | 実態通り |
| 昼夜逆転（昼420min） | [420] | 420 | 0 | 420 | sin/cosに時刻反映 |
| 夕方仮眠+夜間睡眠 | [60, 420] | 420 | 60 | 480 | 正しく分類 |

### HealthKit/Health Connect 取得ウィンドウ

**変更前:** 前日18:00 〜 当日12:00（18時間）
**変更後:** 前日12:00 〜 当日23:59（36時間）

- 36時間ウィンドウで全セグメントを取得
- セグメント単位で保存（統合しない）
- 重複排除: `start` + `end` の組み合わせが一致するものは除外
- セグメント間のギャップが30分未満の場合は1つのブロックとして結合

### バックグラウンド自動同期

`workmanager` パッケージによる定期的なヘルスデータ同期を実装済み。

|項目|内容|
|---|---|
|方式|workmanager 0.5.2 による periodic タスク + アプリ復帰時コールバック|
|同期対象|Apple Health / Health Connect の睡眠・歩数データ|
|頻度|バックグラウンド: OS依存の最小間隔（約15分）、フォアグラウンド: アプリ復帰時に自動実行|
|重複排除|`start` + `end` の組み合わせが一致するセグメントは除外|

### 過去日のヘルスデータ遡及インポート

記入忘れ時に、過去日の歩数・睡眠データを Apple Health / Health Connect から取得する機能を実装済み。

- 詳細入力画面で過去日を選択した際、該当日のヘルスデータをオンデマンドで取得
- 取得ウィンドウは該当日の前日12:00〜当日23:59（通常の同期と同じ36時間）
- 手入力データが既にある場合は上書きしない（ユーザーの意図を尊重）

### ホーム画面ウィジェット

iOS WidgetKit / Android AppWidget を `home_widget` パッケージで実装済み。

|項目|内容|
|---|---|
|表示内容|今日の不調リスク（%）|
|更新タイミング|バッチ予測結果の Firestore 更新を検知して反映|
|未予測時|「学習中」または前回の予測結果を表示|
...
- 自動取得時は全睡眠セグメントを取得し、主睡眠・仮眠を自動分類して表示
- 連携できない場合：就寝時刻・起床時刻を手入力
- 手動入力時は「＋睡眠を追加」ボタンで複数セグメントの入力が可能（仮眠・分割睡眠に対応）
...
- ROC-AUCはログ出力として残す
- **Brier score** を校正指標として各 fold で算出（テスト正例数で重み付け平均）。予測確率と実際の発生率の乖離を検出するためのモニタリング用指標
...
- **1σルール**: LightGBMのCV PR-AUC平均がLRの「平均+1σ」を超えた場合のみLightGBMに切替。差がLRの標準偏差以内であれば「有意差なし」としてシンプルなLRを維持する。ただし **σの上限を0.1にキャップ**（`MODEL_SELECTION_MAX_STD`）する。fold数が少ない・データが不均衡な場合にσが過大になり、LightGBMが実質的に選択不可能になるのを防ぐ
...
|cvFolds           |有効fold数                    |
|brierScore        |Brier score（CV重み付け平均、校正指標）|
...
|順序|処理                    |方法                                                        |
|--|----------------------|----------------------------------------------------------|
|1 |Firestore のユーザーデータ削除  |`users/{uid}` 以下の全サブコレクション（daily、predictions、model_status、feedback、batch_logs）|
...
- **MVP実装**: クライアント側で `daily`, `model_status`, `feedback`, `predictions`, `batch_logs` の全サブコレクションを順次バッチ削除で実装済み（`lib/services/firestore_service.dart`）。Cloud Functions への移行は将来対応
...
|カテゴリ|機能                                  |
|----|------------------------------------|
|記録  |体調5段階入力、睡眠・歩数・ストレス入力、ヘルスケア連携、直近3日編集、過去日のヘルスデータ遡及インポート |
|予測  |今日の不調リスク（14日〜）、3日リスク（60日〜）、信頼度表示    |
|分析  |要因TOP3、具体的数値付きアドバイス、不調の基準表示         |
|可視化 |体調×特徴量グラフ（7日/30日/全期間）、睡眠パターングラフ、日次一覧|
|連携  |ヘルスデータのバックグラウンド自動同期（workmanager）、ホーム画面ウィジェット（WidgetKit / Android Widget）|
|その他 |週次フィードバック                           |
...
|2026-03-07|アカウント削除のMVP実装状況を追記（Section 15.2）                  |クライアント側実装、batch_logs含む全サブコレクション削除対応済み                         |
...
|2026-03-07|config.pyの未使用定数VALIDATION_DAYS=14を記録              |動的計算に移行済み、コードから削除済み（2026-03-13）                               |
...
|2026-03-24|Brier scoreを校正指標として追加（Section 10）                    |予測確率と実際の発生率の乖離をモニタリング。batch_logsにも記録                             |
|2026-03-24|欠損値フォールバックを特徴量別デフォルト値に変更（Section 9）             |`fillna(0)` を廃止し、bed_minutes(23:30)、wake_minutes(07:00)等の妥当なデフォルトに変更    |
|2026-03-24|バックグラウンド自動同期を追記（Section 7.1）                       |workmanager による定期同期 + アプリ復帰時の自動取得                                  |
|2026-03-24|過去日のヘルスデータ遡及インポートを追記（Section 7.1）               |記入忘れ時に過去日の歩数・睡眠をオンデマンドで取得                                       |
