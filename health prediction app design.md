...
|sleep_missing    |睡眠データなし      |欠損フラグ（0 or 1）    |
|sleep_dev        |睡眠(ばらつき)     |直近平均との差          |
|steps_filled     |歩数           |t-1の値、欠損時は過去7日平均 |
|steps_missing    |歩数データなし      |欠損フラグ（0 or 1）    |
...
|2026-03-07|特徴量ラベル対応表をコードの命名に合わせて更新（Section 13, 14.1）         |`steps_t1`→`steps_filled`等、`*_missing`フラグを追加                  |
