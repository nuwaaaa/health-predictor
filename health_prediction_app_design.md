...
2. **不調の"予報"による行動調整** — 予測結果に基づく事前対策
3. **体調データの蓄積と振り返り** — 記録の可視化・傾向把握
...
2. 条件達成（60日 AND 不調10件）：ロジスティック回帰と LightGBM を**両方学習**
...
2. **最適閾値の推定**: 好調日のデータから「リスクが低い行動水準」を算出（好調日の25パーセンタイル値）
...
| 要素 | 内容 | 備考 |
|------|------|------|
| (1) 今日の予測カード | 不調リスク（%）、信頼度、3日リスク（条件達成後） | 14日未満は「学習中（あと○日）」表示 |
